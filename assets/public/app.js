// MoonRay APK v4.1 — v3.3 base + host chain stabil + smart error + success reporting
...
const GENERATORS = {
    ...
    'wan-2-6-i2v': { id: 'wan-2-6-i2v', name: 'WAN 2.6 I2V', icon: '🏔️', category: 'i2v', desc: 'I2V 720p — cepat & terjangkau', endpoint: '/image-to-video/wan-v2-6-720p', imageField: 'image', inputs: ['image', 'prompt'], settings: { duration: { type: 'select', label: 'Durasi', options: [{ value: '5', label: '5s' }, { value: '10', label: '10s' }], default: '5' } } },
    ...
    'runway-4-5': { id: 'runway-4-5', name: 'Runway 4.5', icon: '⚡', category: 'i2v', badge: 'NEW', desc: 'Generasi video kualitas tinggi', endpoint: '/image-to-video/runway-4-5', imageField: 'image', inputs: ['image', 'prompt'], settings: { ratio: { type: 'select', label: 'Ratio', options: [{ value: '1280:720', label: '16:9' }, { value: '720:1280', label: '9:16' }, { value: '960:960', label: '1:1' }], default: '1280:720' }, duration: { type: 'select', numeric: true, label: 'Durasi', options: [{ value: '5', label: '5s' }, { value: '8', label: '8s' }, { value: '10', label: '10s' }], default: '8' } } },
    ...
    'pixverse-v5': { id: 'pixverse-v5', name: 'PixVerse v5', icon: '✨', category: 'i2v', desc: 'Efek video kreatif', endpoint: '/image-to-video/pixverse-v5', imageField: 'image_url', inputs: ['image', 'prompt'], settings: { resolution: { type: 'select', label: 'Resolusi', options: [{ value: '360p', label: '360p' }, { value: '540p', label: '540p' }, { value: '720p', label: '720p' }, { value: '1080p', label: '1080p' }], default: '720p' }, duration: { type: 'select', numeric: true, label: 'Durasi', options: [{ value: '5', label: '5s' }, { value: '8', label: '8s' }, { value: '10', label: '10s' }], default: '5' } } },
    ...
    return new Promise((resolve) => {
        ...
        img.onload = () => {
            try {
                ...
                // Cek resolusi minimum (reject < 300px)
                if (w < 300 || h < 300) {
...
let state = {
    ...
    activeTasks: [],
    completedResults: [],
    history: (() => { try { return JSON.parse(localStorage.getItem('fpk_history') || '[]'); } catch(e) { return []; } })()
};
...
async function reportError(errorType, errorMessage, context = {}) {
    ...
    try {
        ...
        const payload = {
            ...
            platform: 'apk',
            version: device.appVersion ? device.appVersion + '-apk' : 'unknown-apk',
            device: JSON.stringify(device)
        };
        const bodyStr = JSON.stringify(payload);
        // Fire-and-forget — jangan blok user
        // PENTING: Content-Type HARUS text/plain (bukan application/json)
        // karena application/json memicu CORS preflight (OPTIONS) yang
        // Google Apps Script tidak bisa tangani → request gagal diam-diam
        fetch(AUTH_URL, {
            method: 'POST',
            ...
            // Fetch gagal (Android 16 blocked, dll) → coba native bridge
            try {
                if (typeof MoonrayBridge !== 'undefined' && typeof MoonrayBridge.nativeUpload === 'function') {
                    // Kirim sebagai file text agar bisa pakai nativeUpload (multipart)
                    // Apps Script terima di doPost(e) → e.postData.contents
                    const b64 = btoa(unescape(encodeURIComponent(bodyStr)));
                    MoonrayBridge.nativeUpload(b64, AUTH_URL, 'payload', 'error-report.json');
                }
            } catch(ne) { /* silent */ }
        });
    } catch(e) {
...
async function reportSuccess(generator, taskId) {
    if (!AUTH_URL) return;
    try {
        const device = getDeviceInfo();
        const payload = {
            action: 'success-report',
            timestamp: new Date().toISOString(),
            email: localStorage.getItem('mr_email') || 'unknown',
            generator: generator,
            taskId: taskId,
...
function renderContent() {
    ...
    if (state._objectUrls) { state._objectUrls.forEach(u => URL.revokeObjectURL(u)); state._objectUrls = []; }
...
function renderOnboarding(mc) {
    ...
    <div class="card" style="margin-top:16px;text-align:left;border-left:3px solid var(--accent)"><div style="display:flex;align-items:center;gap:10px;margin-bottom:10px"><div style="width:30px;height:30px;border-radius:50%;background:var(--accent-gradient);display:flex;align-items:center;justify-content:center;font-weight:700;font-size:13px;flex-shrink:0;color:#fff">1</div><div><div style="font-weight:600;font-size:13px">Daftar Akun Freepik (Gratis)</div><div style="font-size:11px;color:var(--text-muted)">Dapatkan $5 kredit gratis</div></div></div><a href="#" onclick="event.preventDefault();openExternal('https://www.google.com/search?q=freepik')" style="display:block;text-align:center;padding:10px;background:var(--bg-input);border:1px solid var(--border);border-radius:8px;color:var(--accent);text-decoration:none;font-size:12px;font-weight:600">📝 Buka Halaman Daftar →</a></div>
    <div class="card" style="margin-top:10px;text-align:left;border-left:3px solid var(--warning)"><div style="display:flex;align-items:center;gap:10px;margin-bottom:12px"><div style="width:30px;height:30px;border-radius:50%;background:linear-gradient(135deg,var(--warning),#f97316);display:flex;align-items:center;justify-content:center;font-weight:700;font-size:13px;flex-shrink:0;color:#fff">2</div><div><div style="font-weight:600;font-size:13px">Paste API Key</div><div style="font-size:11px;color:var(--text-muted)">Disimpan lokal di perangkat ini</div></div></div><div style="display:flex;gap:8px"><input type="password" id="onboardKeyInput" class="form-input" placeholder="fpk-xxxxxxxxxxxxx" style="flex:1;font-size:12px"><button class="btn btn-primary" style="white-space:nowrap;padding:8px 14px;font-size:12px" onclick="const k=sanitizeApiKey(document.getElementById('onboardKeyInput').value);if(!k){toast('Masukkan API key','error');return;}state.apiKey=k;localStorage.setItem('fpk_api_key',k);updateKeyStatus();renderContent();toast('API key disimpan! 🎉','success');if(!localStorage.getItem('fpk_disclaimer_accepted'))setTimeout(()=>openDisclaimer(),500);">Simpan</button></div></div>
...
function _fileToBase64ForUpload(file) {
    return new Promise((resolve, reject) => {
        ...
        reader.onerror = () => {
            // Fallback: pakai native bridge readFileAsBase64 (untuk device yang FileReader gagal)
            if (typeof MoonrayBridge !== 'undefined' && MoonrayBridge.readFileAsBase64) {
                try {
                    const b64 = MoonrayBridge.readFileAsBase64(file.name);
                    if (b64 && b64.length > 0) { resolve(b64); return; }
                } catch (e) { /* fall through */ }
            }
            reject(new Error('FileReader failed, no bridge fallback'));
        };
...
async function tryTmpfiles(file) {
    // P1: Upload langsung dari cache (streaming, no OOM, bypass WebView)
    if (typeof MoonrayBridge !== 'undefined' && typeof MoonrayBridge.nativeUploadFromCache === 'function' && typeof MoonrayBridge.hasFile === 'function') {
        try {
            if (MoonrayBridge.hasFile(file.name)) {
                console.log('[Moonray] tryTmpfiles: using nativeUploadFromCache for', file.name);
                const raw = MoonrayBridge.nativeUploadFromCache(file.name, 'https://tmpfiles.org/api/v1/upload', 'file', file.name || 'upload.jpg', '');
                if (raw) { const d = JSON.parse(raw); if (d.status === 'success' && d.data?.url) return d.data.url.replace('tmpfiles.org/', 'tmpfiles.org/dl/'); }
            }
        } catch (e) { console.warn('[Moonray] tryTmpfiles cache upload failed:', e.message); }
    }
    // P2: Native upload via base64 (fallback untuk file yang tidak di cache)
    if (_hasNativeUpload()) {
        try {
...
async function tryCatbox(file) {
    ...
    if (typeof MoonrayBridge !== 'undefined' && typeof MoonrayBridge.nativeUploadFromCache === 'function' && typeof MoonrayBridge.hasFile === 'function') {
        try {
            if (MoonrayBridge.hasFile(file.name)) {
                console.log('[Moonray] tryCatbox: using nativeUploadFromCache for', file.name);
                const raw = MoonrayBridge.nativeUploadFromCache(file.name, 'https://catbox.moe/user/api.php', 'fileToUpload', file.name || 'upload.jpg', 'reqtype=fileupload');
                if (raw && raw.startsWith('https://')) return raw.trim();
            }
        } catch (e) { console.warn('[Moonray] tryCatbox cache upload failed:', e.message); }
    }
    // P2: Native upload via base64 (fallback)
...
async function tryFreeimage(file) {
    ...
    if (typeof MoonrayBridge !== 'undefined' && typeof MoonrayBridge.nativeUploadFromCache === 'function' && typeof MoonrayBridge.hasFile === 'function') {
        try {
            if (MoonrayBridge.hasFile(file.name)) {
                console.log('[Moonray] tryFreeimage: using nativeUploadFromCache for', file.name);
                const raw = MoonrayBridge.nativeUploadFromCache(file.name, 'https://freeimage.host/api/1/upload?key=6d207e02198a847aa98d0a2a901485a5', 'source', file.name || 'upload.jpg', 'type=file&action=upload');
                if (raw) { const d = JSON.parse(raw); if (d.image?.url) return d.image.url; }
            }
        } catch (e) { console.warn('[Moonray] tryFreeimage cache upload failed:', e.message); }
    }
    // P2: Native upload via base64 (fallback)
    if (_hasNativeUpload() && typeof MoonrayBridge.nativeUploadWithFields === 'function') {
        try {
...
async function tryImgur(file) {
    // Imgur anonymous upload — gambar only (JPG/PNG/GIF), max 20MB
    // TERBUKTI: Freepik bisa akses URL i.imgur.com (test 18 April 2026)
    // ⚠️ CATATAN: Imgur butuh header Authorization yang native bridge tidak support
    // Jadi Imgur HANYA bisa dipakai via fetch (browser). Di Android WebView = CORS blocked.
    // Imgur adalah FALLBACK, bukan host utama.
    const fd = new FormData();
    fd.append('image', file);
    const r = await safeFetch('https://api.imgur.com/3/image', {
        method: 'POST',
        headers: { 'Authorization': 'Client-ID 546c25a59c58ad7' },
        body: fd
    }, uploadTimeout(file));
    const d = await r.json();
    if (d.success && d.data?.link) return d.data.link;
    return null;
}

async function tryLitterbox(file) {
    // Litterbox (catbox temp) — semua format termasuk video, 24h expiry
    // TERBUKTI: Freepik bisa akses URL litter.catbox.moe (test 18 April 2026)
    // P1: Upload langsung dari cache (streaming, no OOM, bypass WebView)
    if (typeof MoonrayBridge !== 'undefined' && typeof MoonrayBridge.nativeUploadFromCache === 'function' && typeof MoonrayBridge.hasFile === 'function') {
        try {
            if (MoonrayBridge.hasFile(file.name)) {
                console.log('[Moonray] tryLitterbox: using nativeUploadFromCache for', file.name);
                const raw = MoonrayBridge.nativeUploadFromCache(file.name, 'https://litterbox.catbox.moe/resources/internals/api.php', 'fileToUpload', file.name || 'upload.jpg', 'reqtype=fileupload&time=24h');
                if (raw && raw.startsWith('https://')) return raw.trim();
            }
        } catch (e) { console.warn('[Moonray] tryLitterbox cache upload failed:', e.message); }
    }
    // P2: Native upload via base64 (fallback untuk file yang tidak di cache, misal compressed image)
    if (_hasNativeUpload() && typeof MoonrayBridge.nativeUploadWithFields === 'function') {
        try {
            console.log('[Moonray] tryLitterbox: using native upload');
            const b64 = await _fileToBase64ForUpload(file);
            const raw = MoonrayBridge.nativeUploadWithFields(b64, 'https://litterbox.catbox.moe/resources/internals/api.php', 'fileToUpload', file.name || 'upload.jpg', 'reqtype=fileupload&time=24h');
            if (raw && raw.startsWith('https://')) return raw.trim();
        } catch (e) { console.warn('[Moonray] tryLitterbox native failed:', e.message); }
    }
    // P3: Fallback fetch biasa
    const fd = new FormData();
    ...
    const r = await safeFetch('https://litterbox.catbox.moe/resources/internals/api.php', { method: 'POST', body: fd }, uploadTimeout(file));
    const url = await r.text();
    return (url && url.startsWith('https://')) ? url.trim() : null;
}

async function tryUguu(file) {
    // Uguu — semua format termasuk video, 48h expiry
    // P1: Upload langsung dari cache (streaming, no OOM, bypass WebView)
    if (typeof MoonrayBridge !== 'undefined' && typeof MoonrayBridge.nativeUploadFromCache === 'function' && typeof MoonrayBridge.hasFile === 'function') {
        try {
            if (MoonrayBridge.hasFile(file.name)) {
                console.log('[Moonray] tryUguu: using nativeUploadFromCache for', file.name);
                const raw = MoonrayBridge.nativeUploadFromCache(file.name, 'https://uguu.se/upload', 'files[]', file.name || 'upload.jpg', '');
                if (raw) {
                    try { const d = JSON.parse(raw); if (d.success && d.files && d.files[0] && d.files[0].url) return d.files[0].url; } catch (pe) { /* not JSON */ }
                }
            }
        } catch (e) { console.warn('[Moonray] tryUguu cache upload failed:', e.message); }
    }
    // P2: Native bridge upload via base64 (fallback)
    if (_hasNativeUpload()) {
        try {
            console.log('[Moonray] tryUguu: using native upload');
            const b64 = await _fileToBase64ForUpload(file);
            const raw = MoonrayBridge.nativeUpload(b64, 'https://uguu.se/upload', 'files[]', file.name || 'upload.jpg');
            if (raw) {
                try {
                    const d = JSON.parse(raw);
                    if (d.success && d.files && d.files[0] && d.files[0].url) return d.files[0].url;
                } catch (pe) { /* not JSON, ignore */ }
            }
        } catch (e) { console.warn('[Moonray] tryUguu native failed:', e.message); }
    }
    // P3: Fallback fetch biasa
    const fd = new FormData();
    fd.append('files[]', file, file.name);
    const r = await safeFetch('https://uguu.se/upload', { method: 'POST', body: fd }, uploadTimeout(file));
    const d = await r.json();
    if (d.success && d.files?.[0]?.url) return d.files[0].url;
    return null;
}
...
async function try0x0st(file) {
    ...
    if (typeof MoonrayBridge !== 'undefined' && typeof MoonrayBridge.nativeUploadFromCache === 'function' && typeof MoonrayBridge.hasFile === 'function') {
        try {
            if (MoonrayBridge.hasFile(file.name)) {
                console.log('[Moonray] try0x0st: using nativeUploadFromCache for', file.name);
                const raw = MoonrayBridge.nativeUploadFromCache(file.name, 'https://0x0.st', 'file', file.name || 'upload.jpg', '');
                if (raw && raw.startsWith('https://')) return raw.trim();
            }
        } catch (e) { console.warn('[Moonray] try0x0st cache upload failed:', e.message); }
    }
    // P2: Native upload via base64 (fallback)
    if (_hasNativeUpload()) {
        try {
...
async function tryX0at(file) {
    // x0.at — semua format, ~3 hari expiry, Freepik CONFIRMED ✅ (30 Apr 2026)
    if (typeof MoonrayBridge !== 'undefined' && typeof MoonrayBridge.nativeUploadFromCache === 'function' && typeof MoonrayBridge.hasFile === 'function') {
        try {
            if (MoonrayBridge.hasFile(file.name)) {
                console.log('[Moonray] tryX0at: using nativeUploadFromCache for', file.name);
                const raw = MoonrayBridge.nativeUploadFromCache(file.name, 'https://x0.at/', 'file', file.name || 'upload.jpg', '');
                if (raw && raw.startsWith('https://')) return raw.trim();
            }
        } catch (e) { console.warn('[Moonray] tryX0at cache upload failed:', e.message); }
    }
    if (_hasNativeUpload()) {
        try {
            console.log('[Moonray] tryX0at: using native upload');
            const b64 = await _fileToBase64ForUpload(file);
            const raw = MoonrayBridge.nativeUpload(b64, 'https://x0.at/', 'file', file.name || 'upload.jpg');
            if (raw && raw.startsWith('https://')) return raw.trim();
        } catch (e) { console.warn('[Moonray] tryX0at native failed:', e.message); }
    }
    const fd = new FormData(); fd.append('file', file);
    const r = await safeFetch('https://x0.at/', { method: 'POST', body: fd }, uploadTimeout(file));
    const url = await r.text();
    return (url && url.startsWith('https://')) ? url.trim() : null;
}

async function tryTempsh(file) {
    // temp.sh — semua format, ~1 hari expiry, Freepik CONFIRMED ✅ (27 Apr 2026)
    if (typeof MoonrayBridge !== 'undefined' && typeof MoonrayBridge.nativeUploadFromCache === 'function' && typeof MoonrayBridge.hasFile === 'function') {
        try {
            if (MoonrayBridge.hasFile(file.name)) {
                console.log('[Moonray] tryTempsh: using nativeUploadFromCache for', file.name);
                const raw = MoonrayBridge.nativeUploadFromCache(file.name, 'https://temp.sh/', 'file', file.name || 'upload.jpg', '');
                if (raw && raw.startsWith('https://')) return raw.trim();
            }
        } catch (e) { console.warn('[Moonray] tryTempsh cache upload failed:', e.message); }
    }
    if (_hasNativeUpload()) {
        try {
            console.log('[Moonray] tryTempsh: using native upload');
            const b64 = await _fileToBase64ForUpload(file);
            const raw = MoonrayBridge.nativeUpload(b64, 'https://temp.sh/', 'file', file.name || 'upload.jpg');
            if (raw && raw.startsWith('https://')) return raw.trim();
        } catch (e) { console.warn('[Moonray] tryTempsh native failed:', e.message); }
    }
    const fd = new FormData(); fd.append('file', file);
    const r = await safeFetch('https://temp.sh/', { method: 'POST', body: fd }, uploadTimeout(file));
    const url = await r.text();
    return (url && url.startsWith('https://')) ? url.trim() : null;
}
...
async function showUploadDiagnostic(file, hostErrors) {
    let report = '📋 MOONRAY DIAGNOSTIK v5\n';
    report += '━━━━━━━━━━━━━━━━━━━━━━━━\n\n';
    // Device info
    const ua = navigator.userAgent || 'unknown';
    ...
    // Network test
    report += '🌐 Network Test:\n';
    const testUrls = ['https://litterbox.catbox.moe', 'https://uguu.se', 'https://catbox.moe'];
...
// Helper: kumpulkan info native bridge untuk diagnostik error report
// Semua di-wrap try/catch agar TIDAK PERNAH menyebabkan error
function _getBridgeDiag(file) {
    try {
        if (typeof MoonrayBridge === 'undefined') return 'noBridge';
        const name = file?.name || '';
        const hasFile = MoonrayBridge.hasFile ? MoonrayBridge.hasFile(name) : '?';
        const size = MoonrayBridge.getFileSize ? MoonrayBridge.getFileSize(name) : '?';
        return `bridge:has=${hasFile},sz=${size}`;
    } catch(e) { return 'bridge:err'; }
}
async function uploadImageToHost(file, statusCallback) {
    const hosts = [{ name: 'litterbox', fn: () => tryLitterbox(file) }, { name: 'uguu', fn: () => tryUguu(file) }, { name: 'x0at', fn: () => tryX0at(file) }, { name: 'tempsh', fn: () => tryTempsh(file) }, { name: 'imgur', fn: () => tryImgur(file) }, { name: 'tmpfiles', fn: () => tryTmpfiles(file) }, { name: 'freeimage', fn: () => tryFreeimage(file) }, { name: 'catbox', fn: () => tryCatbox(file) }];
    const hostErrors = [];
    ...
    reportError('upload_image_fail', 'Semua host gagal', { step: 'upload_image', fileInfo: `${file.name}|${file.size}|${file.type}|${_getBridgeDiag(file)}`, hostResults: JSON.stringify(hostErrors.map(h => h.name + ':' + h.result)).substring(0, 400), generator: state.activeGenerator });
    await showUploadDiagnostic(file, hostErrors);
    return null;
}
async function uploadVideoToHost(file, statusCallback) {
    const hosts = [{ name: 'litterbox', fn: () => tryLitterbox(file) }, { name: 'uguu', fn: () => tryUguu(file) }, { name: 'x0at', fn: () => tryX0at(file) }, { name: 'tempsh', fn: () => tryTempsh(file) }, { name: 'tmpfiles', fn: () => tryTmpfiles(file) }, { name: 'catbox', fn: () => tryCatbox(file) }];
    const hostErrors = [];
    for (let i = 0; i < hosts.length; i++) {
        const host = hosts[i];
        try {
    ...
    reportError('upload_video_fail', 'Semua host gagal', { step: 'upload_video', fileInfo: `${file.name}|${file.size}|${file.type}|${_getBridgeDiag(file)}`, hostResults: JSON.stringify(hostErrors.map(h => h.name + ':' + h.result)).substring(0, 400), generator: state.activeGenerator });
    await showUploadDiagnostic(file, hostErrors);
    return null;
}
...
async function generate() {
    ...
    const prepTask = { id: tempTaskId, genId: gen.id, genName: gen.name, outputType: gen.outputType || 'video', progress: 1, statusText: 'Mempersiapkan...' };
    ...
    if (gen.inputs.includes('image') && processedImageFile && !imageUrl) {
        ...
        prepTask.statusText = 'Memproses gambar...'; prepTask.progress = 2; updateActiveTasksUI();
    ...
    if (gen.imageBase64 && gen.inputs.includes('image') && processedImageFile && !imageUrl) {
        prepTask.statusText = 'Konversi gambar...'; prepTask.progress = 3; updateActiveTasksUI();
    ...
    let _imageJustUploaded = false, _videoJustUploaded = false;
    if (!gen.imageBase64 && gen.inputs.includes('image') && processedImageFile && !imageUrl) { prepTask.statusText = 'Uploading gambar...'; prepTask.progress = 2; prepTask._uploading = true; updateActiveTasksUI(); const _upIv = setInterval(() => { if (prepTask.progress < 5) { prepTask.progress++; updateActiveTasksUI(); } }, 3000); imageUrl = await uploadImageToHost(processedImageFile, (msg) => { prepTask.statusText = msg; updateActiveTasksUI(); }); clearInterval(_upIv); prepTask._uploading = false; if (!imageUrl) { removeTask(tempTaskId); return; } prepTask.progress = 5; prepTask.statusText = 'Gambar berhasil diupload'; updateActiveTasksUI(); state._uploadedPreviewUrls.image = imageUrl; _imageJustUploaded = true; }
    if (gen.inputs.includes('video') && state.uploadedFiles.video && !videoUrl) { prepTask.statusText = 'Uploading video...'; prepTask.progress = 2; prepTask._uploading = true; updateActiveTasksUI(); const _upIv2 = setInterval(() => { if (prepTask.progress < 5) { prepTask.progress++; updateActiveTasksUI(); } }, 3000); videoUrl = await uploadVideoToHost(state.uploadedFiles.video, (msg) => { prepTask.statusText = msg; updateActiveTasksUI(); }); clearInterval(_upIv2); prepTask._uploading = false; if (!videoUrl) { removeTask(tempTaskId); return; } prepTask.progress = 5; prepTask.statusText = 'Video berhasil diupload'; updateActiveTasksUI(); state._uploadedPreviewUrls.video = videoUrl; _videoJustUploaded = true; }
    if (gen.inputs.includes('audio') && state.uploadedFiles.audio && !audioUrl) { prepTask.statusText = 'Uploading audio...'; prepTask.progress = 2; prepTask._uploading = true; updateActiveTasksUI(); const _upIv3 = setInterval(() => { if (prepTask.progress < 5) { prepTask.progress++; updateActiveTasksUI(); } }, 3000); audioUrl = await uploadVideoToHost(state.uploadedFiles.audio, (msg) => { prepTask.statusText = msg; updateActiveTasksUI(); }); clearInterval(_upIv3); prepTask._uploading = false; if (!audioUrl) { removeTask(tempTaskId); return; } prepTask.progress = 5; prepTask.statusText = 'Audio berhasil diupload'; updateActiveTasksUI(); state._uploadedPreviewUrls.audio = audioUrl; }

    // Langkah 2: Verifikasi URL — HANYA untuk URL cache/user input (bukan yang baru diupload)
    if (imageUrl && processedImageFile && !_imageJustUploaded) {
    ...
    if (videoUrl && state.uploadedFiles.video && !_videoJustUploaded) {
    ...
    recordRequest();
    prepTask.statusText = 'Mengirim ke API...'; prepTask.progress = 5; updateActiveTasksUI();
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        try {
            if (attempt > 0) {
                ...
                prepTask.progress = 5; updateActiveTasksUI();
            ...
            if (!response.ok) {
                ...
                if (response.status === 401 || response.status === 403) {
                    ...
                    if (rawMsg.includes('IP') && rawMsg.includes('blocked')) {
                        errMsg = '🌐 IP WiFi Anda terblokir. Coba gunakan data seluler (matikan WiFi), atau gunakan VPN.';
                    } else if (rawMsg.includes('disabled') || rawMsg.includes('abuse')) {
                        errMsg = '⚠️ Akun Freepik dinonaktifkan. Buat API key baru di freepik.com/api';
                    } else {
                        errMsg = 'API key tidak valid atau expired. (HTTP ' + response.status + ')';
                    }
                }
            ...
            if (taskId) {
                removeTask(tempTaskId);
                const task = { id: taskId, genId: gen.id, genName: gen.name, outputType: gen.outputType || 'video', progress: 6, statusText: 'Queued...', _pollStartTime: Date.now() };
...
async function pollTask(task, gen) {
    ...
    const poll = async () => {
        ...
        try {
            ...
            // Time-based progress: 6-90% (7min), 90-95% (3min), 96-99% (1%/min)
            const _elapsed = (Date.now() - (task._pollStartTime || Date.now())) / 1000;
            let _timedPct;
            if (_elapsed < 420) { _timedPct = 6 + Math.floor(_elapsed / 5); } // 6→90% dalam 420s (7 menit), naik 1% per 5 detik
            else if (_elapsed < 600) { _timedPct = 90 + Math.floor((_elapsed - 420) / 36); } // 90→95% dalam 180s (3 menit)
            else { _timedPct = Math.min(99, 96 + Math.floor((_elapsed - 600) / 60)); } // 96→99%, 1% per menit
            task.progress = Math.max(task.progress, progress, _timedPct);
            task.statusText = status === 'IN_QUEUE' ? 'Dalam antrian...' : status === 'IN_PROGRESS' ? `Processing... ${Math.round(task.progress)}%` : status;
            updateActiveTasksUI();
            if (status === 'COMPLETED' || status === 'completed') { removeTask(task.id); handleResult(data, task.outputType, task.genName); addToHistory(task.genName, 'COMPLETED', data, task.outputType); toast(`✅ ${task.genName} selesai!`, 'success'); reportSuccess(task.genId, task.id); }
...
function buildTaskCardHtml(task) { const pct = Math.round(task.progress || 1); const uploadClass = task._uploading ? ' uploading' : ''; return `<div class="card" style="margin-bottom:8px;padding:10px 14px"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:5px"><span style="font-size:12px;font-weight:600;color:var(--accent)"><span class="spinner" style="width:12px;height:12px;border-width:2px;vertical-align:middle;margin-right:4px"></span>${task.genName}</span><span style="font-size:11px;font-weight:600;color:var(--accent)">${pct}%</span></div><div class="progress-bar"><div class="progress-fill${uploadClass}" style="width:${pct}%"></div></div><div style="font-size:10px;color:var(--text-muted);margin-top:3px">${task.statusText || 'Processing...'}</div></div>`; }
...
// ==================== TOAST ====================
function toast(message, type = 'info') { if (type === 'error' && document.getElementById('resultArea')) { showError(message); return; } const container = document.getElementById('toasts'); const el = document.createElement('div'); el.className = `toast toast-${type}`; el.textContent = message; container.appendChild(el); setTimeout(() => { el.style.opacity = '0'; setTimeout(() => el.remove(), 300); }, 4000); }

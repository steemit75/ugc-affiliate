...
const GENERATORS = {
    ...
    'minimax-hailuo-02': { id: 'minimax-hailuo-02', name: 'Hailuo 02', icon: '🌊', category: 'i2v', desc: 'Gerakan manusia ultra-realistis, 1080p', endpoint: '/image-to-video/minimax-hailuo-02-1080p', imageField: 'first_frame_image', inputs: ['image', 'prompt'], settings: {} },
    'minimax-hailuo-2-3': { id: 'minimax-hailuo-2-3', name: 'Hailuo 2.3', icon: '🌊', category: 'i2v', badge: 'NEW', desc: 'Versi terbaru Minimax', endpoint: '/image-to-video/minimax-hailuo-2-3-1080p', imageField: 'first_frame_image', inputs: ['image', 'prompt'], settings: {} },
    ...
    'seedance-pro': { id: 'seedance-pro', name: 'Seedance Pro', icon: '💃', category: 'i2v', desc: 'ByteDance model', endpoint: '/image-to-video/seedance-pro-1080p', imageField: 'image', inputs: ['image', 'prompt'], settings: {} },
    'pixverse-v5': { id: 'pixverse-v5', name: 'PixVerse v5', icon: '✨', category: 'i2v', desc: 'Efek video kreatif', endpoint: '/image-to-video/pixverse-v5', inputs: ['image', 'prompt'], settings: { resolution: { type: 'select', label: 'Resolusi', options: [{ value: '360p', label: '360p' }, { value: '540p', label: '540p' }, { value: '720p', label: '720p' }, { value: '1080p', label: '1080p' }], default: '720p' }, duration: { type: 'select', numeric: true, label: 'Durasi', options: [{ value: '5', label: '5s' }, { value: '8', label: '8s' }, { value: '10', label: '10s' }], default: '5' } } },
...
function handleDrop(e, type) { e.preventDefault(); document.getElementById('zone_' + type).classList.remove('dragover'); const f = e.dataTransfer.files[0]; if (f) { state.uploadedFiles[type] = f; renderContent(); } }
function handleFileSelect(e, type) { const f = e.target.files[0]; if (f) { state.uploadedFiles[type] = f; renderContent(); } }
...
async function tryTmpfiles(file) { const fd = new FormData(); fd.append('file', file); const r = await safeFetch('https://tmpfiles.org/api/v1/upload', { method: 'POST', body: fd }); const d = await r.json(); return (d.status === 'success' && d.data?.url) ? d.data.url.replace('tmpfiles.org/', 'tmpfiles.org/dl/') : null; }
async function tryCatbox(file) { const fd = new FormData(); fd.append('reqtype', 'fileupload'); fd.append('fileToUpload', file); const r = await safeFetch('https://catbox.moe/user/api.php', { method: 'POST', body: fd }); const url = await r.text(); return (url && url.startsWith('https://')) ? url.trim() : null; }
async function tryFreeimage(file) { const fd = new FormData(); fd.append('source', file); fd.append('type', 'file'); fd.append('action', 'upload'); const r = await safeFetch('https://freeimage.host/api/1/upload?key=6d207e02198a847aa98d0a2a901485a5', { method: 'POST', body: fd }); const d = await r.json(); return d.image?.url || null; }
async function tryLabsStorage(file) { if (!STORAGE_URL) return null; const fd = new FormData(); fd.append('file', file); const r = await safeFetch(STORAGE_URL + '/upload', { method: 'POST', body: fd }); const d = await r.json(); return d.success ? d.url : null; }
async function try0x0st(file) { const fd = new FormData(); fd.append('file', file); const r = await safeFetch('https://0x0.st', { method: 'POST', body: fd }); const url = await r.text(); return (url && url.startsWith('https://')) ? url.trim() : null; }
...
async function uploadImageToHost(file, statusCallback) {
    ...
    for (let i = 0; i < hosts.length; i++) {
        ...
        try {
            ...
            const url = await host.fn(); if (url) { console.log(`[Moonray] ✅ ${host.name}:`, url); return url; }
        } catch (e) { console.warn(`[Moonray] ${host.name} failed:`, e); }
    }
    toast('Upload gagal. Coba lagi.', 'error'); return null;
}
async function uploadVideoToHost(file, statusCallback) {
    ...
    toast('Upload video gagal. Coba lagi.', 'error'); return null;
}
...
async function generate() {
    ...
    if (gen.imageBase64 && gen.inputs.includes('image') && state.uploadedFiles.image && !imageUrl) {
        prepTask.statusText = 'Konversi gambar...'; prepTask.progress = 10; updateActiveTasksUI();
        try { imageBase64Data = await fileToBase64(state.uploadedFiles.image); console.log('[Moonray] Image converted to base64, length:', imageBase64Data.length); } catch (e) { console.error('[Moonray] Base64 error:', e); removeTask(tempTaskId); toast('Gagal konversi gambar', 'error'); return; }
    }
    if (!gen.imageBase64 && gen.inputs.includes('image') && state.uploadedFiles.image && !imageUrl) { prepTask.statusText = 'Uploading gambar...'; prepTask.progress = 10; updateActiveTasksUI(); imageUrl = await uploadImageToHost(state.uploadedFiles.image, (msg) => { prepTask.statusText = msg; updateActiveTasksUI(); }); if (!imageUrl) { removeTask(tempTaskId); return; } state._uploadedPreviewUrls.image = imageUrl; }
    ...
    const poll = async () => {
        ...
        try {
            ...
            if (status === 'COMPLETED' || status === 'completed') { removeTask(task.id); handleResult(data, task.outputType, task.genName); addToHistory(task.genName, 'COMPLETED', data, task.outputType); toast(`✅ ${task.genName} selesai!`, 'success'); }
                ...
                const errDetail = data?.data?.error || data?.data?.message || data?.data?.reason || data?.data?.detail || data?.error || data?.message || '';
                ...
                if (!errDetail && task._failRetries < MAX_FAIL_RETRIES) {
                    ...
                    // Sudah retry atau ada error detail → gagalkan
                    removeTask(task.id);
                    showError(`${task.genName} gagal: ${errDetail || 'Server tidak memberikan detail error'}`);

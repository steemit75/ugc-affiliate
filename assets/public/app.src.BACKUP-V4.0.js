// MoonRay APK v4.0 — Rollback v3.3 + host chain baru + adaptive polling
    ...
    return new Promise((resolve) => {
        ...
        img.onload = () => {
            try {
                ...
                // Auto-upscale: gambar 100-299px → upscale ke 300px minimum
                if (w < 100 || h < 100) {
                    resolve({ success: false, reason: `Resolusi gambar terlalu kecil (${img.naturalWidth}×${img.naturalHeight}px, min 100×100px).` });
                    return;
                }
                if (w < 300 || h < 300) {
                    console.log('[Moonray] Auto-upscale:', w, 'x', h, '→ min 300px');
                    const scale = Math.max(300 / w, 300 / h);
...
async function uploadImageToHost(file, statusCallback) {
    const hosts = [{ name: 'uguu', fn: () => tryUguu(file) }, { name: 'x0at', fn: () => tryX0at(file) }, { name: 'tempsh', fn: () => tryTempsh(file) }, { name: 'litterbox', fn: () => tryLitterbox(file) }, { name: 'catbox', fn: () => tryCatbox(file) }, { name: 'freeimage', fn: () => tryFreeimage(file) }, { name: 'tmpfiles', fn: () => tryTmpfiles(file) }];
    const hostErrors = [];
...
async function uploadVideoToHost(file, statusCallback) {
    const hosts = [{ name: 'tempsh', fn: () => tryTempsh(file) }, { name: 'uguu', fn: () => tryUguu(file) }, { name: 'catbox', fn: () => tryCatbox(file) }, { name: 'x0at', fn: () => tryX0at(file) }, { name: 'litterbox', fn: () => tryLitterbox(file) }, { name: 'tmpfiles', fn: () => tryTmpfiles(file) }];
    const hostErrors = [];
    for (let i = 0; i < hosts.length; i++) {
        const host = hosts[i];
        try {
...
async function generate() {
    ...
    // v4.0: Parallel upload untuk Motion Control (image + video bersamaan)
    const needImageUpload = !gen.imageBase64 && gen.inputs.includes('image') && processedImageFile && !imageUrl;
    const needVideoUpload = gen.inputs.includes('video') && state.uploadedFiles.video && !videoUrl;
    if (needImageUpload && needVideoUpload) {
        prepTask.statusText = 'Uploading gambar & video...'; prepTask.progress = 10; updateActiveTasksUI();
        const [imgResult, vidResult] = await Promise.all([
            uploadImageToHost(processedImageFile, (msg) => { prepTask.statusText = '📸 ' + msg; updateActiveTasksUI(); }),
            uploadVideoToHost(state.uploadedFiles.video, (msg) => { prepTask.statusText = '🎥 ' + msg; updateActiveTasksUI(); })
        ]);
        if (!imgResult) { removeTask(tempTaskId); return; }
        if (!vidResult) { removeTask(tempTaskId); return; }
        imageUrl = imgResult; videoUrl = vidResult;
        state._uploadedPreviewUrls.image = imageUrl;
        state._uploadedPreviewUrls.video = videoUrl;
    } else {
        if (needImageUpload) { prepTask.statusText = 'Uploading gambar...'; prepTask.progress = 10; updateActiveTasksUI(); imageUrl = await uploadImageToHost(processedImageFile, (msg) => { prepTask.statusText = msg; updateActiveTasksUI(); }); if (!imageUrl) { removeTask(tempTaskId); return; } state._uploadedPreviewUrls.image = imageUrl; }
        if (needVideoUpload) { prepTask.statusText = 'Uploading video...'; prepTask.progress = 25; updateActiveTasksUI(); videoUrl = await uploadVideoToHost(state.uploadedFiles.video, (msg) => { prepTask.statusText = msg; updateActiveTasksUI(); }); if (!videoUrl) { removeTask(tempTaskId); return; } state._uploadedPreviewUrls.video = videoUrl; }
    }
    ...
    const poll = async () => {
        ...
        try {
            ...
            if (status === 'COMPLETED' || status === 'completed') { removeTask(task.id); handleResult(data, task.outputType, task.genName); addToHistory(task.genName, 'COMPLETED', data, task.outputType); toast(`✅ ${task.genName} selesai!`, 'success'); reportSuccess(task.genId, task.id); }
                ...
                // v4.0: Adaptive polling — mulai 3s, naik ke max 15s (hemat ~60% request)
                const interval = Math.min(15000, 3000 + (attempts * 500));
                setTimeout(poll, interval);
            }
        } catch (err) { console.warn('[Moonray] Poll error:', err); setTimeout(poll, 10000); }
    };

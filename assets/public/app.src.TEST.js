// MoonRay APK v3.1 — Fix Scoped Storage upload (Vivo Y35)
...
async function uploadImageToHost(file, statusCallback) {
    const hosts = [{ name: 'litterbox', fn: () => tryLitterbox(file) }, { name: 'uguu', fn: () => tryUguu(file) }, { name: 'imgur', fn: () => tryImgur(file) }, { name: 'tmpfiles', fn: () => tryTmpfiles(file) }, { name: '0x0', fn: () => try0x0st(file) }, { name: 'freeimage', fn: () => tryFreeimage(file) }, { name: 'catbox', fn: () => tryCatbox(file) }];
    const hostErrors = [];
...
async function uploadVideoToHost(file, statusCallback) {
    const hosts = [{ name: 'litterbox', fn: () => tryLitterbox(file) }, { name: 'uguu', fn: () => tryUguu(file) }, { name: 'tmpfiles', fn: () => tryTmpfiles(file) }, { name: '0x0', fn: () => try0x0st(file) }, { name: 'catbox', fn: () => tryCatbox(file) }];
    const hostErrors = [];
    for (let i = 0; i < hosts.length; i++) {
        const host = hosts[i];
        try {

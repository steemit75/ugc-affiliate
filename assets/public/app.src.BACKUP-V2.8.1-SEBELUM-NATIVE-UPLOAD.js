// MoonRay APK v2.7 — Fix upload Android 11+
...
async function tryTmpfiles(file) { const fd = new FormData(); fd.append('file', file); const r = await safeFetch('https://tmpfiles.org/api/v1/upload', { method: 'POST', body: fd }, uploadTimeout(file)); const d = await r.json(); return (d.status === 'success' && d.data?.url) ? d.data.url.replace('tmpfiles.org/', 'tmpfiles.org/dl/') : null; }
async function tryCatbox(file) { const fd = new FormData(); fd.append('reqtype', 'fileupload'); fd.append('fileToUpload', file); const r = await safeFetch('https://catbox.moe/user/api.php', { method: 'POST', body: fd }, uploadTimeout(file)); const url = await r.text(); return (url && url.startsWith('https://')) ? url.trim() : null; }
async function tryFreeimage(file) { const fd = new FormData(); fd.append('source', file); fd.append('type', 'file'); fd.append('action', 'upload'); const r = await safeFetch('https://freeimage.host/api/1/upload?key=6d207e02198a847aa98d0a2a901485a5', { method: 'POST', body: fd }, uploadTimeout(file)); const d = await r.json(); return d.image?.url || null; }
async function tryLabsStorage(file) { if (!STORAGE_URL) return null; const fd = new FormData(); fd.append('file', file); const r = await safeFetch(STORAGE_URL + '/upload', { method: 'POST', body: fd }, uploadTimeout(file)); const d = await r.json(); return d.success ? d.url : null; }
async function try0x0st(file) { const fd = new FormData(); fd.append('file', file); const r = await safeFetch('https://0x0.st', { method: 'POST', body: fd }, uploadTimeout(file)); const url = await r.text(); return (url && url.startsWith('https://')) ? url.trim() : null; }

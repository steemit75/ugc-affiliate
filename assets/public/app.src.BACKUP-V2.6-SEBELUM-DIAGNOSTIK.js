// Simpan fetch ASLI sebelum CapacitorHttp mempatch-nya (fix upload gagal di beberapa device)
const _originalFetch = (typeof window.fetch === 'function') ? window.fetch.bind(window) : null;
...
// ==================== FILE UPLOAD TO HOST ====================
async function safeFetch(url, options = {}, timeoutMs = 30000) { const c = new AbortController(); const t = setTimeout(() => c.abort(), timeoutMs); try { const fetchFn = _originalFetch || fetch; const r = await fetchFn(url, { ...options, signal: c.signal }); clearTimeout(t); return r; } catch (err) { clearTimeout(t); throw err.name === 'AbortError' ? new Error('Timeout') : err; } }
